...
public class AppointmentServiceImpl implements AppointmentService {
	...
	@Override
	public AppointmentEntity getAppointmentByPatientAndDoctor(
            Long patientId,
            Long doctorId
    ) {
        return appointmentRepository
                ...
                .orElseThrow(() ->
                        new RuntimeException(
                            "No appointment exists for selected patient & doctor"
                        )
                );
    }
	...
	for(DoctorAvailabilityResponseDto a : avail) {
		...
		while(slotStart.plusMinutes(SLOT_DURATION_MINUTES).isBefore(a.getEndTime()) ||  slotStart.plusMinutes(SLOT_DURATION_MINUTES).equals(a.getEndTime())) {
			...
			boolean isAvailable = app.stream().noneMatch(ap -> currentSlotStart.isBefore(ap.getEndTime()) && currentSlotEnd.isAfter(ap.getStartTime())
					);
	...
	public HoldSlotResponseDto holdSlot(@Valid HoldSlotRequestDto req) {
		...
		if(appointmentRepository.existsByDoctorIdAndAppointmentDateAndStartTimeAndEndTimeAndStatusIn(
				req.getDoctorId(),req.getDate(),req.getStartTime(),req.getEndTime(),List.of(AppointmentStatus.PENDING,AppointmentStatus.SCHEDULED))
				){
	...
	public boolean markAsScheduled(Long appointmentId) {
		...
		AppointmentEntity appointmentEntity = appointmentRepository.findById(appointmentId).orElseThrow(()-> new RuntimeException("No Such Appointment Found"));
